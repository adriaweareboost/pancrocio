...
let fromAddress = 'Scan&Boost <scanboost@weareboost.online>';
...
const EMAIL_STRINGS: Record<string, EmailStrings> = {
  es: {
    ...
    verifySubtitle: 'Introduce este código en Scan&Boost para desbloquear tu informe CRO:',
    verifyExpiry: 'El código expira cuando se usa. Si no solicitaste este código, ignora este email.',
    verifySubject: 'Tu código Scan&Boost',
  ...
  en: {
    ...
    verifySubtitle: 'Enter this code in Scan&Boost to unlock your CRO report:',
    verifyExpiry: 'This code expires after use. If you didn\'t request it, ignore this email.',
    verifySubject: 'Your Scan&Boost code',
  ...
  fr: {
    ...
    verifySubtitle: 'Entrez ce code dans Scan&Boost pour débloquer votre rapport CRO :',
    verifyExpiry: 'Ce code expire après utilisation. Si vous ne l\'avez pas demandé, ignorez cet email.',
    verifySubject: 'Votre code Scan&Boost',
  ...
  de: {
    ...
    verifySubtitle: 'Geben Sie diesen Code in Scan&Boost ein, um Ihren CRO-Bericht freizuschalten:',
    verifyExpiry: 'Der Code verfällt nach Nutzung. Falls Sie ihn nicht angefordert haben, ignorieren Sie diese E-Mail.',
    verifySubject: 'Ihr Scan&Boost-Code',
  ...
  it: {
    ...
    verifySubtitle: 'Inserisci questo codice in Scan&Boost per sbloccare il tuo report CRO:',
    verifyExpiry: 'Il codice scade dopo l\'uso. Se non lo hai richiesto, ignora questa email.',
    verifySubject: 'Il tuo codice Scan&Boost',
  ...
  pt: {
    ...
    verifySubtitle: 'Insira este código no Scan&Boost para desbloquear seu relatório CRO:',
    verifyExpiry: 'O código expira após o uso. Se você não solicitou, ignore este email.',
    verifySubject: 'Seu código Scan&Boost',
...
function emailWrapper(content: string): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><style>body{margin:0;padding:0;background:#f8f9fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#46495C}a{color:#EC5F29}</style></head><body><div style="max-width:560px;margin:0 auto;padding:32px 20px"><div style="text-align:center;margin-bottom:24px"><h1 style="font-size:24px;font-weight:800;color:#070F2D;margin:0">Scan&amp;<span style="color:#EC5F29">Boost</span></h1></div><div style="background:white;border-radius:16px;padding:32px;box-shadow:0 4px 24px rgba(7,15,45,0.08)">${content}</div><div style="text-align:center;margin-top:24px;font-size:12px;color:#9ca3af"><p>Scan&amp;Boost &middot; Powered by <a href="https://www.weareboost.online" style="color:#070F2D;text-decoration:none;font-weight:700">Boost</a></p><p style="margin-top:4px"><a href="https://www.weareboost.online" style="color:#9ca3af">weareboost.online</a></p></div></div></body></html>`;
}
...
export async function sendLeadNotification(
  ...
  const html = emailWrapper(`
    <h2 style="font-size:18px;color:#070F2D;margin:0 0 12px">Nuevo lead en Scan&Boost</h2>
  ...
  return send(notifyTo, `[Scan&Boost] Nuevo lead: ${email}${scoreInfo}`, html);
}
